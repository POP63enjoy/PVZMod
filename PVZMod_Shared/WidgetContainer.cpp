...
WidgetContainer::WidgetContainer()
{
	...
	mUpdateIterator = Iterator<WidgetList>(mWidgets.end(), &mWidgets);
...
void WidgetContainer::RemoveWidget(Widget* theWidget)
{
	...
	if (anItr != mWidgets.end())
	{
		...
		bool erasedCur = (anItr == mUpdateIterator.iter);
		mWidgets.erase(anItr++);
		if (erasedCur)
		{
			mUpdateIterator.iter = anItr;
...
void WidgetContainer::BringToFront(Widget* theWidget)
{
	...
	if (anItr != mWidgets.end())
	{
		if (anItr == mUpdateIterator.iter)
		{
			mUpdateIterator.iter++;
			mUpdateIteratorModified = true;
		}
...
void WidgetContainer::UpdateAll(ModalFlags* theFlags)
{
	...
	mUpdateIterator = Iterator<WidgetList>(mWidgets.begin(), &mWidgets);

	while (mUpdateIterator.iter != mWidgets.end())
	{
		mUpdateIteratorModified = false;

		Widget* aWidget = *mUpdateIterator.iter;
