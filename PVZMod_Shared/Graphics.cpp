...
Image& GraphicsState::mStaticImage = *(Image*)0x732984;
...
void Graphics::DrawImage(Image* theImage, int theX, int theY)
{
	...
	theX = (int)(theX + mTransX);
	theY = (int)(theY + mTransY);
...
void Graphics::DrawImage(Image* theImage, int theX, int theY, const Rect& theSrcRect)
{
	...
	if (mScaleX != 1 || mScaleY != 1)
	{
		Rect aDestRect((int)(mScaleOrigX + floor((theX - mScaleOrigX) * mScaleX)), (int)(mScaleOrigY + floor((theY - mScaleOrigY) * mScaleY)), (int)(ceil(theSrcRect.mWidth * mScaleX)), (int)(ceil(theSrcRect.mHeight * mScaleY)));
...
void Graphics::DrawImageRotated(Image* theImage, int theX, int theY, double theRot, const Rect* theSrcRect)
{
	if (theSrcRect == NULL)
	{
		...
		DrawImageRotatedF(theImage, (float)theX, (float)theY, theRot, (float)aRotCenterX, (float)aRotCenterY, theSrcRect);
	}
	else
	{
...
void Graphics::DrawImageTransformHelper(Image* theImage, const Transform& theTransform, const Rect& theSrcRect, float x, float y, bool useFloat)
{
	...
	if (theTransform.mHaveRot)
	{
		...
		if (useFloat)
			...
			DrawImageRotated(theImage, (int)x, (int)y, theTransform.mRot, (int)rx, (int)ry, &theSrcRect);
	}
