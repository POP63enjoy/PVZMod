...
PakInterface* PVZMod::gPakInterface = (PakInterface*)0x76B530;
PakInterfaceBase* PVZMod::gPakInterfaceBase = gPakInterface;

static String StringToUpper(const String& theString)
{
	String aString;
...
bool PakInterface::AddPakFile(const String& theFileName)
{
	HANDLE aFileHandle = CreateFileA(theFileName.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
	...
	aPakRecord->mFileName = theFileName.c_str();
...
bool PakInterface::AddPakRes(HMODULE theModule,const String& theName, const char* theResName, const char* theResType)
{
	HRSRC aResHandle = FindResourceA(theModule, theResName, theResType);
	if (aResHandle == NULL)
		return false;
	
	HGLOBAL aResData = LoadResource(theModule, aResHandle);
	if (aResData == NULL)
		return false;
	
	int aResSize = SizeofResource(theModule, aResHandle);
	if (aResSize == 0)
		return false;
	
	void* aPtr = LockResource(aResData);
	
	mPakCollectionList.push_back(PakCollection());
	PakCollection* aPakCollection = &mPakCollectionList.back();

	aPakCollection->mFileHandle = nullptr;
	aPakCollection->mMappingHandle = nullptr;
	aPakCollection->mDataPtr = aPtr;

	PakRecordMap::iterator aRecordItr = mPakRecordMap.insert(PakRecordMap::value_type(StringToUpper(theName), PakRecord())).first;
	PakRecord* aPakRecord = &(aRecordItr->second);
	aPakRecord->mCollection = aPakCollection;
	aPakRecord->mFileName = theName.c_str();
	aPakRecord->mStartPos = 0;
	aPakRecord->mSize = aResSize;

	PFILE* aFP = FOpen(theName.c_str(), "rb");
	if (aFP == NULL)
		return false;
...
PFILE* PakInterface::FOpen(const char* theFileName, const char* anAccess)
{
	...
	FILE* aFP = nullptr;
	fopen_s(&aFP, theFileName, anAccess);
	if (aFP == NULL)
		return NULL;
	PFILE* aPFP = new PFILE;
	aPFP->mRecord = NULL;
...
bool PakInterface::PFindNext(PFindData* theFindData, LPWIN32_FIND_DATAA lpFindFileData)
{
			...
			{				
					...
					aFileName + strlen(aFileName) - (theFindData->mFindCriteria.length() - aStarPos) + 1) == 0))
				{
					...
					if (aLastSlashPos == -1)
						...
						strcpy_s(lpFindFileData->cFileName, anItr->second.mFileName.c_str());
					else
						//strcpy(lpFindFileData->cFileName, anItr->second.mFileName.c_str() + aLastSlashPos + 1);
						strcpy_s(lpFindFileData->cFileName, anItr->second.mFileName.c_str() + aLastSlashPos + 1);
...
HANDLE PakInterface::FindFirstFileA(const char* lpFileName, LPWIN32_FIND_DATAA lpFindFileData)
{
	...
	aFindData->mWHandle = ::FindFirstFileA(aFindData->mFindCriteria.c_str(), lpFindFileData);
...
BOOL PakInterface::FindNextFileA(HANDLE hFindFile, LPWIN32_FIND_DATAA lpFindFileData)
{
	...
	return ::FindNextFileA(aFindData->mWHandle, lpFindFileData);
}
